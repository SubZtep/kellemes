...
import { QAPair, SearchResult, EmbeddedQAPair } from '../types';
...
export class RAGService {
  ...
  constructor() {
    this.vectorService = new VectorService(
      `${config.vectorDb.path}/qa_vectors.json`
    );
  }
  ...
  async retrieve(query: string, topK?: number): Promise<SearchResult[]> {
    ...
    const results = this.vectorService.search(
      queryEmbedding,
      k,
  ...
  private isMedicalQuery(query: string, sources: SearchResult[]): boolean {
    ...
    const nonMedicalKeywords = /\b(nvidia|gpu|computer|gaming|software|programming|code|app|phone|company|business|brand|product|movie|game|sports|football|basketball|music|song|video|youtube|tiktok|instagram|facebook|twitter)\b/i;
   ...
   */
  async generateResponse(query: string, topK?: number): Promise<{
    response: string;
    sources: SearchResult[];
  }> {
    // Check if query is casual/conversational (short greetings, etc.)
    const casualPatterns = /^(hi|hey|hello|howdy|sup|what's up|how are you|good morning|good afternoon|good evening|thanks|thank you|bye|goodbye)\??!?$/i;
